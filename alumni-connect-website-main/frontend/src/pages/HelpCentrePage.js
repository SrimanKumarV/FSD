...
const GuideCard = ({ icon: Icon, color, label, desc }) => (
  <motion.div
    whileHover={{ y: -3 }}
    className="p-5 rounded-2xl border border-gray-200 dark:border-slate-700/50 bg-white dark:bg-slate-900 hover:border-gray-300 dark:hover:border-slate-600 transition-colors cursor-default"
  >
...
const FaqItem = ({ q, a }) => {
  ...
  return (
    <div className="border-b border-gray-200 dark:border-slate-700/50 last:border-0">
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-start justify-between py-4 text-left hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors gap-4"
      >
        <span className={`text-sm font-medium transition-colors ${open ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-900 dark:text-white'}`}>{q}</span>
        {open ? <ChevronUp className="w-4 h-4 text-gray-500 dark:text-slate-500 flex-shrink-0 mt-0.5" />
               : <ChevronDown className="w-4 h-4 text-gray-500 dark:text-slate-500 flex-shrink-0 mt-0.5" />}
      </button>
      <AnimatePresence>
        {open && (
          <motion.div
          ...
          >
            <p className="text-sm text-gray-600 dark:text-slate-400 leading-relaxed pb-4">{a}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-10">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <BookOpen className="w-5 h-5 text-indigo-600 dark:text-indigo-400" /> Quick Guides
          </h2>
        </div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
        <h2 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2 mb-4">
          <Zap className="w-5 h-5 text-cyan-600 dark:text-cyan-400" /> Frequently Asked Questions
        </h2>
          ...
          {categories.map(cat => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-semibold border transition-all ${
                activeCategory === cat
                  ? 'bg-indigo-50 dark:bg-indigo-500/20 border-indigo-200 dark:border-indigo-500 text-indigo-700 dark:text-indigo-300'
                  : 'border-gray-200 dark:border-slate-700/50 text-gray-600 dark:text-slate-500 hover:border-gray-300 dark:hover:border-slate-600 hover:text-gray-900 dark:hover:text-slate-300'
              }`}
            >{cat}</button>
          ))}
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-4">
            {filtered.map(section => (
              <div key={section.category} className="rounded-2xl border border-gray-200 dark:border-slate-700/50 bg-white dark:bg-slate-900 px-5 shadow-sm">
                <p className="text-xs font-bold text-gray-500 dark:text-slate-500 uppercase tracking-widest pt-4 pb-2">{section.category}</p>
      ...
      <motion.div
        ...
        className="rounded-3xl p-8 border border-gray-200 dark:border-slate-700/50 bg-white dark:bg-slate-900 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-6"
      >
        <div>
          <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-1">Still need help?</h3>
          <p className="text-sm text-gray-500 dark:text-slate-400">Can't find what you're looking for? Submit feedback or send us an email.</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3 flex-shrink-0">
          <Link
            to="/feedback"
            className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-semibold rounded-xl transition-colors whitespace-nowrap shadow-sm"
          >
