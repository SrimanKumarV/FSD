...
const ProjectCollaboration = () => {
  ...
  // Fetch projects (Global)
  const { data, isLoading } = useQuery(
    ['all-projects', search],
    () => {
      return api.get('/projects' + (search ? `?search=${search}` : '')).then(res => res.data);
    },
        ...
        <div className="max-w-7xl mx-auto space-y-6 pt-4 px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-gray-600 dark:text-gray-300">Discover and collaborate on amazing projects built by our community</p>
            </div>
            {user && (
              <button
