  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-2 text-gray-600 dark:text-gray-400">Manage your platform and monitor system performance</p>
      </div>
...
const SystemSettingsTab = () => {
  ...
  const { data: engagementData, isLoading: loadingEngagement, refetch: refetchEngagement } = useQuery(
    ['admin-engagement-stats'],
    () => api.get('/admin/engagement-stats').then(res => res.data),
    { staleTime: 30000 }
  );
  ...
  const handleSendTestEmail = async () => {
    ...
    try {
      const res = await api.post('/admin/trigger-weekly-engagement', { testEmail });
  ...
  const handleTriggerAll = async () => {
    const eligible = engagementData?.stats?.digestSubscribers || 0;
    if (!window.confirm(`Trigger weekly engagement digest to all ${eligible} eligible users now? This will also register fresh activity with Brevo SMTP.`)) {
      return;
    }
    setIsTriggering(true);
    try {
      const res = await api.post('/admin/trigger-weekly-engagement', { forceAll: true });
      toast.success(res.data.message || 'Weekly engagement job completed!');
      refetchEngagement();
    } catch (err) {
      toast.error(err.response?.data?.message || 'Failed to trigger weekly digest');
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Weekly Engagement Digest & Brevo SMTP Automation Card */}
      <motion.div 
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        className="glass-card rounded-2xl border border-white/20 dark:border-gray-700/50 p-6 relative overflow-hidden"
      >
        ...
        <div className="relative z-10 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-100 dark:border-gray-800 pb-4">
            <div>
              <h3 className="text-xl font-bold text-gray-900 dark:text-white flex items-center">
                ...
                Weekly Engagement Digest & Brevo SMTP Keep-Alive
              </h3>
              <p className="text-gray-600 dark:text-gray-400 mt-1 text-sm">
                Automated weekly opportunity emails sent to keep users engaged and prevent Brevo SMTP key expiration.
              </p>
            </div>
            <button
              onClick={() => refetchEngagement()}
              disabled={loadingEngagement}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg transition-colors self-start sm:self-auto"
            >
              <RefreshCw className={`w-3.5 h-3.5 ${loadingEngagement ? 'animate-spin' : ''}`} />
              Refresh Stats
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="bg-white/60 dark:bg-gray-900/60 p-4 rounded-xl border border-gray-200/70 dark:border-gray-700/70">
              <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 block uppercase">Subscribers</span>
              <span className="text-2xl font-black text-indigo-600 dark:text-indigo-400 mt-1 block">
                {loadingEngagement ? '...' : engagementData?.stats?.digestSubscribers ?? 0}
              </span>
              <span className="text-[11px] text-gray-500 mt-0.5 block">Eligible recipients</span>
            </div>

            <div className="bg-white/60 dark:bg-gray-900/60 p-4 rounded-xl border border-amber-200/70 dark:border-amber-900/40">
              <span className="text-xs font-semibold text-amber-600 dark:text-amber-400 block uppercase">Inactive Users</span>
              <span className="text-2xl font-black text-amber-600 dark:text-amber-400 mt-1 block">
                {loadingEngagement ? '...' : engagementData?.stats?.inactiveUsers ?? 0}
              </span>
              <span className="text-[11px] text-gray-500 mt-0.5 block">Inactive for ≥ 7 days</span>
            </div>

            <div className="bg-white/60 dark:bg-gray-900/60 p-4 rounded-xl border border-emerald-200/70 dark:border-emerald-900/40">
              <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 block uppercase">Active (7d)</span>
              <span className="text-2xl font-black text-emerald-600 dark:text-emerald-400 mt-1 block">
                {loadingEngagement ? '...' : Math.max(0, (engagementData?.stats?.totalUsers || 0) - (engagementData?.stats?.inactiveUsers || 0))}
              </span>
              <span className="text-[11px] text-gray-500 mt-0.5 block">Active within 7 days</span>
            </div>

            <div className="bg-white/60 dark:bg-gray-900/60 p-4 rounded-xl border border-blue-200/70 dark:border-blue-900/40">
              <span className="text-xs font-semibold text-blue-600 dark:text-blue-400 block uppercase">SMTP Status</span>
              <div className="mt-1 flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse"></span>
                <span className="text-sm font-bold text-gray-900 dark:text-white truncate">
                  {loadingEngagement ? '...' : engagementData?.stats?.emailProvider || 'Active'}
                </span>
              </div>
              <span className="text-[11px] text-gray-500 mt-0.5 block">Mondays @ 10:00 AM</span>
            </div>
          </div>

          {/* Action Row */}
          <div className="bg-gradient-to-r from-indigo-50/70 to-purple-50/70 dark:from-indigo-950/20 dark:to-purple-950/20 p-5 rounded-xl border border-indigo-100 dark:border-indigo-900/50 space-y-4">
            <h4 className="text-sm font-bold text-gray-900 dark:text-white flex items-center gap-2">
              <Zap className="w-4 h-4 text-indigo-600 dark:text-indigo-400" />
              Test & Immediate Dispatch Controls
            </h4>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3.5 py-2.5 text-sm bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none"
                  />
                  <button
                    onClick={handleSendTestEmail}
                    disabled={isSendingTest}
                    className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-lg transition-colors disabled:opacity-50 flex items-center gap-1.5 whitespace-nowrap shadow-sm"
                  >
                    {isSendingTest ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Send className="w-3.5 h-3.5" />}
                    Send Preview
                  </button>
                </div>
                <span className="text-[11px] text-gray-500 mt-1 block">Sends a sample of the weekly inactivity & opportunities digest.</span>
              </div>

              {/* Immediate Trigger All */}
              <div className="flex flex-col justify-end">
                <button
                  onClick={handleTriggerAll}
                  disabled={isTriggering}
                  className="w-full px-5 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white text-sm font-bold rounded-lg transition-all shadow-md shadow-emerald-600/20 disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {isTriggering ? (
                    <>
                      <RefreshCw className="w-4 h-4 animate-spin" />
                      Dispatched Digest to All Users...
                    </>
                  ) : (
                    <>
                      <Mail className="w-4 h-4" />
                      Trigger Weekly Digest to All Users Now
                    </>
                  )}
                </button>
                <span className="text-[11px] text-gray-500 mt-1 text-center block">
                  Instantly sends digest & registers fresh activity with Brevo SMTP.
                </span>
              </div>
            </div>

            {/* CLI Tip */}
            <div className="pt-2 border-t border-indigo-100/60 dark:border-indigo-900/40 text-xs text-gray-600 dark:text-gray-400 flex items-center justify-between flex-wrap gap-2">
              <span>💡 <strong>Tip for Server Sleep:</strong> If your server sleeps on free hosting, run via terminal: <code className="bg-white dark:bg-gray-900 px-1.5 py-0.5 rounded font-mono text-[11px] border border-gray-200 dark:border-gray-800">npm run send:weekly-digest</code></span>
            </div>
          </div>
        </div>
      </motion.div>
