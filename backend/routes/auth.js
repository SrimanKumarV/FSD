...
router.post('/register', [
  ...
  try {
    ...
    // Add role-specific information, supporting both nested objects and flat body params
    if (role === 'student') {
      userFields.studentInfo = {
        course: studentInfo?.course || req.body.course || '',
        year: studentInfo?.year || (req.body.year ? Number(req.body.year) : undefined),
        university: studentInfo?.university || req.body.university || '',
        expectedGraduation: studentInfo?.expectedGraduation || (req.body.graduationYear ? Number(req.body.graduationYear) : undefined)
      };
      userFields.isApproved = true;
    } else if (role === 'alumni') {
      userFields.alumniInfo = {
        graduationYear: alumniInfo?.graduationYear || (req.body.graduationYear ? Number(req.body.graduationYear) : undefined),
        company: alumniInfo?.company || req.body.company || '',
        position: alumniInfo?.position || req.body.position || '',
        industry: alumniInfo?.industry || req.body.industry || '',
        experience: alumniInfo?.experience || (req.body.experience ? Number(req.body.experience) : 0)
      };
      // Auto approve alumni for seamless usage while allowing admin review
      userFields.isApproved = true;
    }
