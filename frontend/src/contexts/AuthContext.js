...
export const AuthProvider = ({ children }) => {
  ...
  // Update user profile with backend API call
  const updateUser = async (userData, role) => {
    try {
      let response;
      if (role) {
        response = await api.put(`/users/profile/${role}`, userData[role + 'Info'] || userData);
      } else {
  ...
  // Fetch and sync LinkedIn profile data
  const fetchLinkedInData = async (linkedinUrl) => {
    try {
      const response = await api.post('/users/linkedin/fetch', { linkedinUrl });
      dispatch({
        type: AUTH_ACTIONS.UPDATE_USER,
        payload: response.data.user
      });
      toast.success(response.data.message || 'LinkedIn profile data synced!');
      return { success: true, user: response.data.user };
    } catch (error) {
      const message = error.response?.data?.message || 'Failed to fetch LinkedIn profile';
      toast.error(message);
      return { success: false, error: message };
    }
  };
